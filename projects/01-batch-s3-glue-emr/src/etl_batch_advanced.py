...
def transform(df):
    ...
    return (
        ...
        .withColumn(
            "is_peak", F.col("hour").isin([7, 8, 9, 17, 18, 19]).cast("boolean")
        )
    )
...
def main(args):
    spark = get_spark()
    df = spark.read.schema(SCHEMA).option("header", True).csv(args.input)
    ...
    (
        out.repartition(args.partitions)  # more partitions for bigger data
        .write.mode("overwrite" if args.overwrite else "append")
        .partitionBy("dt")
        .parquet(args.output)
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--since", help='inclusive lower bound timestamp, e.g. "2025-09-01 00:00:00"'
    )
    ap.add_argument(
        "--until", help='exclusive upper bound timestamp, e.g. "2025-09-02 00:00:00"'
    )
