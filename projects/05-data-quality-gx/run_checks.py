...
def run(parquet_path, expectations_file):
    ...
    for e in exp:
        ...
        if t == "expect_table_row_count_to_be_between":
            ok = (len(df) >= k.get("min_value", 0)) and (
                k.get("max_value") is None or len(df) <= k["max_value"]
            )
        elif t == "expect_column_values_to_not_be_null":
            ok = df[k["column"]].notna().all()
        elif t == "expect_column_values_to_be_between":
            col = df[k["column"]]
            ok = (col >= k.get("min_value", col.min())).all() and (
                k.get("max_value") is None or (col <= k["max_value"]).all()
            )
        else:
            ok = True
